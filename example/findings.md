...
paperterm v0.1 真实实现的 anthropic provider（plan §6.2）应该用 AST 提取
...
`example/prompt.md` 后期新增的 chunked 协议（每轮 ACK + 终轮合并 + 跨轮
...
P1. **manual provider 行号准确性 17% 错误率**（本节 §4 实测）。
    ...
    **修复方向**：plan §6.2 anthropic provider 改用 AST 注入 prose 时
...
- **行号准确性是 manual provider 的真实瓶颈**（~17% 错误率），佐证
  plan §6.2 anthropic provider + AST 路径的必要性。

paperterm v0.1 写代码时的具体调整（按 ROI 排序）：

1. **AST 化的 line prefix 注入**（解决 P1，最高影响） — `bootstrap.py`
   走 anthropic provider 时强制把每行 prose prefix 成 `<file>:<line>:`，
   消除 LLM 自数行的失误来源。
...
7. 保留 chunked input mode 在 `prompts.py` 的 standalone 版本里（plan
   §8.4 的人工镜像），但 anthropic provider 走 AST + 内部 chunking，不
   依赖 LLM 跨轮状态。
